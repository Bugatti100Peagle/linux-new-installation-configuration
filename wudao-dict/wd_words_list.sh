...
case $1 in
    -a)
        sed '/^\s*$/d' $filename|sort -u | awk '$1 ~/["`date '+%Y'`"]/ ' | mail  -s "All_new words" 1342720800@qq.com
    ...
    *)
        ...
        #获取时间戳行号
        sed -n "${day_last},${day_now}p" $filename |sort -u | awk '$1 ~/["`date '+%Y'`"]/ ' | mail  -s " `sed -n "${day_last}p" $filename`_to_`sed -n "${day_now}p" $filename`_words" 1342720800@qq.com
